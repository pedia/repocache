...
def local_simple_package_info(package, version=''):
  ...
  if version:
    files = [f for f in files if f['filename'].endswith('%s.tar.gz' % version)]
  if not files:
    return app.config["pypi"].get_simple_package_info(package, version)
  return render_template(
      "simple_package.html",
...
@app.route("/packages/<python_version>/<firstletter>/<package>/<filename>",
           methods=["GET"])
@app.route("/packages/source/<firstletter>/<package>/<filename>",
           methods=["GET"])
def get_file(package, filename, python_version=None, firstletter=None):
  logging.debug("Request to get package with: {0} {1} {2} {3}".format(
      firstletter, package, filename, python_version))
  try:
    response = make_response(app.config["cache"].get_file(
        package, filename, python_version=python_version))
...
def post_uploadpackage():
  ...
  package = re.match(r"(?P<package>(?:[^-]|-[^0-9])*)-[0-9].*\..*",
                     filename).groupdict()["package"]
  logging.debug("Parsed {0!r} out of {1!r}".format(package, filename))
  app.config["package_store"].add_file(package, filename,
                                       request.files[fileparam].stream)
  return jsonify({"uploaded": "ok"})
...
def POST_requirements_txt():
  ...
  if "requirements" not in request.files:
    ...
    response.status_code = 400
    return response
  return jsonify(app.config["cache"].cache_requirements_txt(
      request.files["requirements"].stream))
