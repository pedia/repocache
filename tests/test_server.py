...
from pypicache import cache, disk, exceptions, pypi, server
...
class ServerTestCase(unittest.TestCase):
  ...
  def test_local_package(self):
    ...
    self.assertIn(
        b"""<a href="/packages/mypackage/mypackage-1.0.tar.gz#md5=ahashabcdef">mypackage-1.0.tar.gz</a>""",
        response.body)
  ...
  def test_packages_source_sdist(self):
    for url in [
      ...
      self.mock_packagecache.get_file.assert_called_with("MyPackage",
  ...
  def test_packages_source_notfound(self):
    ...
    self.app.get("/packages/source/m/mypackage/mypackage-1.1.tar.gz",
                 status=404)
    self.mock_packagecache.get_file.assert_called_with("mypackage",
                                                       "mypackage-1.1.tar.gz",
                                                       python_version=None)

  @unittest.skip("Need to figure out PUT under flask")
  def test_put_package_file(self):
    response = self.app.put("/packages/source/m/mypackage/mypackage-1.0.tar.gz",
                            "--package-data--")
  ...
  def test_post_packge_file(self):
    response = self.app.post("/uploadpackage/",
                             upload_files=[("sdist", "mypackage-1.0.tar.gz",
                                            b"--package-data--")])
    self.assertDictEqual(response.json, {"uploaded": "ok"})
  ...
  def test_post_requirements_txt(self):
    ...
    response = self.app.post("/requirements.txt",
                             upload_files=[("requirements", "requirements.txt",
                                            b"mypackage==1.0")])
